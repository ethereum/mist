                        ...
                        got(
                            ...
                            .then(response => {
                                parseJson(response.body, (err, data) => {
                                    ...
                                    blobs =
                                        data.EnumerationResults.Blobs[0].Blob;
                                });

                                // For each platform/arch in clientBinaries.json
                                _.keys(geth.platforms).forEach(platform => {
                                    _.keys(geth.platforms[platform]).forEach(
                                        arch => {
                                            // Update URL
                                            let url =
                                                geth.platforms[platform][arch]
                                                    .download.url;
                                            url = url.replace(
                                                /\d+\.\d+\.\d+-[a-z0-9]{8}/,
                                            ...
                                            geth.platforms[platform][
                                                arch
                                            ].commands.sanity.output[1] = String(
                                                latestGethVersion
                                            );
            ...
            files.forEach(file => {
                if (
                    ...
                    if (error) {
                        error.forEach(diff => {
                            if (
                                diff.typeOfComparedParent !==
                                    diff.typeOfParent &&
                                diff.parent !==
                                    "en.mist.applicationMenu.view.languages" &&
                                diff.parent !==
                                    "en.mist.applicationMenu.view.langCodes"
                            ) {
                                eval(
