...
mistAPIBackend = function(event) {
    ...
    if (event.channel === "mistAPI_sound") {
        sound.pause();
        sound.src = Blaze._escape(
            "file://" + dirname + "/sounds/" + arg + ".mp3"
        );
        sound.play();
    }
        ...
        arg.forEach(function(eventArg) {
            ...
            if (eventArg.action === "addMenu") {
                ...
                if (eventArg.entry.id) {
                    query.$set["menu." + eventArg.entry.id + ".id"] =
                        eventArg.entry.id;
                }

                query.$set[
                    "menu." + eventArg.entry.id + ".selected"
                ] = !!eventArg.entry.selected;
