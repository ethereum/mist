            ...
            if (remoteLookup) {
                https
                        ...
                        function(response) {
                            ...
                            response.on("end", function() {
                                ...
                                if (responseData.results.length) {
                                    resolve(
                                        responseData.results[0].text_signature
                                    );
                                } else {
                    ...
                    .on("error", function(error) {
                        console.warn(
                            "Error querying Function Signature Registry.",
                            err
                        );
...
Template["popupWindows_sendTransactionConfirmation"].onCreated(function() {
    ...
    this.autorun(function() {
        if (
            TemplateVar.get("estimatedGas") >
            Number(TemplateVar.get("providedGas"))
        ) {
            ...
            web3.eth.estimateGas(estimateData, function(e, res) {
                ...
                if (!e && res) {
                    ...
                    Tracker.nonreactive(function() {
                        var gas = Number(
                            TemplateVar.get(template, "providedGas")
                        );

                        if (res === defaultEstimateGas) {
                            return TemplateVar.set(
                                template,
                                "estimatedGas",
                                "invalid"
                            );
...
Template["popupWindows_sendTransactionConfirmation"].helpers({
    ...
    */
    showFormattedParams: function() {
        return (
            TemplateVar.get("params") && TemplateVar.get("displayDecodedParams")
        );
    },
    /**
    ...
    "submit form": function(e, template) {
        ...
        web3.personal.sendTransaction(data, pw || "", function(e, res) {
            ...
            if (!e && res) {
                ...
                if (
                    e.message.indexOf(
                        "Unable to connect to socket: timeout"
                    ) !== -1
                ) {
                    GlobalNotification.warning({
                        content: TAPi18n.__(
                    ...
                    e.message.indexOf("multiple keys match address") !== -1
                ) {
                    GlobalNotification.warning({
                        content: TAPi18n.__(
