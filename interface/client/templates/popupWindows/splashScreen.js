            ...
            if (data !== false) {
                ...
                if (web3.net.peerCount > 0) {
                    ...
                    if (
                        ...
                        translationString =
                            "mist.startScreen.nodeSyncInfoStates";
                    } else {
                ...
                lastSyncData.highestBlock = numeral(
                    lastSyncData.highestBlock
                ).format("0,0");
    ...
    */
    syncStatus: function() {
        ...
        template._intervalId = Meteor.setInterval(function() {
            ...
            var syncData = TemplateVar.get(
                template,
                "lastSyncData",
                lastSyncData
            );
            var translationString = TemplateVar.get(
                template,
                "syncStatusMessage"
            );
            ...
            if (!(syncData._displayBlock > -1)) {
                ...
                syncData._displayKnownStates = Number(
                    syncData.knownStates || 0
                );
            } else {
                // Increment each them slowly to match target number
                syncData._displayBlock +=
                    (Number(syncData.currentBlock) - syncData._displayBlock) /
                    100;
                syncData._displayState +=
                    (Number(syncData.pulledStates || 0) -
                        syncData._displayState) /
                    100;
                syncData._displayKnownStates +=
                    (Number(syncData.knownStates || 0) -
                        syncData._displayKnownStates) /
                    100;
            }
            ...
            if (0 < lastSyncData._displayKnownStates) {
                stateProgress =
                    lastSyncData._displayState /
                    lastSyncData._displayKnownStates *
                    100;
            }

            var progress =
                (lastSyncData._displayBlock -
