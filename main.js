...
function checkTimeSync() {
    if (!Settings.skiptimesynccheck) {
        timesync.checkEnabled((err, enabled) => {
            if (err) {
                log.error(
                    "Couldn't infer if computer automatically syncs time.",
                    err
                );
                return;
            }
                ...
                dialog.showMessageBox(
                    {
                        ...
                        detail: `${global.i18n.t(
                            ...
                            `mist.errors.timeSync.${process.platform}`
                        )}`
                    },
                    () => {}
                );
...
function initializeTabs() {
    ...
    const sortedTabs =
        Tabs.getDynamicView("sorted_tabs") ||
        Tabs.addDynamicView("sorted_tabs");
