...
class Manager extends EventEmitter {
    ...
    _checkForNewConfig(restart) {
        ...
        return got(BINARY_URL, {
            ...
            .catch(err => {
                log.warn(
                    "Error fetching client binaries config from repo",
                    err
                );
            })
            .then(latestConfig => {
                ...
                const platform = process.platform
                    ...
                    .replace("sunos", "linux");
                const binaryVersion =
                    latestConfig.clients[nodeType].platforms[platform][
                        process.arch
                    ];
                const checksums = _.pick(
                    binaryVersion.download,
                    "sha256",
                    "md5"
                );
                    ...
                    dialog.showMessageBox(
                        {
                            ...
                            message: global.i18n.t(
                                "mist.errors.nodeChecksumMismatch.title"
                            ),
