...
class EthereumNode extends EventEmitter {
    ...
    restart(newType, newNetwork, syncMode) {
        return Q.try(() => {
            if (!this.isOwnNode) {
                throw new Error(
                    "Cannot restart node since it was started externally"
                );
            }
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            .then(() => {
                return this.__startNode(nodeType, network, syncMode).catch(
                    err => {
                        ethereumNodeLog.error("Failed to start node", err);
    ...
    _loadDefaults() {
        ...
        this.defaultNodeType =
            Settings.nodeType ||
            Settings.loadUserData("node") ||
            DEFAULT_NODE_TYPE;
        this.defaultNetwork =
            Settings.network ||
            Settings.loadUserData("network") ||
            DEFAULT_NETWORK;
        this.defaultSyncMode =
            Settings.syncmode ||
