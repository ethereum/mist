...
class IpcProviderBackend {
    ...
    _destroyConnection(event) {
        ...
        if (this._connections[ownerId]) {
            ...
            this._connections[ownerId].owner.send(
                "ipcProvider-setWritable",
                false
            );
                        ...
                        if (item.socket.isConnected) {
                            return item.socket.disconnect().then(() => {
                                log.debug(
                                    `Tell owner ${
                                        item.id
                                    } that socket is not currently writeable`
                                );
    ...
    _sendRequest(isSync, event, payload) {
        ...
        return Q.try(() => {
            ...
            .then(conn => {
                ...
                const nonErrorPayloads = _.filter(
                    finalPayloadList,
                    p => !p.error
                );
